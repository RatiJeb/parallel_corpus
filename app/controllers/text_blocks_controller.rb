class TextBlocksController < ApplicationController
  ...
  def search
    ...
    @text_block_pairs = @text_block_pairs.includes(collection: :group).search(params[:query]).order(:original_id)
  ...
  def advanced_search
    ...
    @text_block_pairs = Views::TextBlockPair
    @text_block_pairs = @text_block_pairs.includes(collection: :group)
                          .search(params[:query], params[:termin].present? && params[:termin] != '0')
  ...
  def set_search_params
    @search = Search::TextBlockPair.new(params.permit(:query, :termin, :original_language, :year_start, :year_end,
                                                      :translation_year_start, :translation_year_end,
                                                      search_text_block_pair: { supergroup_ids: [], group_ids: [],
                                                                                collection_ids: [], type_ids: [],
                                                                                field_ids: [], genre_ids: [],
